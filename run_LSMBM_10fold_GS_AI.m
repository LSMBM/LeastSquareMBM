...
% Initialize a cell array to hold best gama and sigma values for each fold
bestParams = cell(kfold, 3);

% Loop through each fold
for i = 1:kfold
    % Load training and testing data for the current fold
    % The dataset has been partitioned into 10 fold using the crossfold_generator function
    train = readmatrix(['training', num2str(i), '.xlsx']);
    test = readmatrix(['testing', num2str(i), '.xlsx']);
    ...
    addpath('LS-SVM-Toolbox');

    % LSMBM training model using grid search
    for j = 1:length(gam)
        for k = 1:length(sig)
            % Adjust the weights for each gamma
            gamwtr=gam(j) .* w_bpnn;
